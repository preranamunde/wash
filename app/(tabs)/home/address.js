...
const address = () => {
    ...
    const renderButtons = () => {
        ...
        return next6Days.map((date, index) => (
            <TouchableOpacity
                ...
                onPress={() => setDeliveryDate(date)} key={index}>
                <Text
                    style={{
                        textAlign: "center",
                        marginTop: 3,
    ...
    const renderPickUpTimeOptions = () => {
                ...
                >
                    {PickUpTimeOptions.map((option, index) => {
                        ...
                        const endTime = moment(
                            `${selectedDate.format("YYYY-MM-DD")} ${option.endTime}`,
                            "YYYY-MM-DD LT"
                        );

                        // Check if the slot is in the past
                        const isTimeSlotPast = isCurrentDate && currentTime.isAfter(endTime);
                        ...
                        return (
                            <TouchableOpacity
                                ...
                                onPress={() => {
                                    if (!isTimeSlotPast) {
                                        setSelectedTime(option); // Select valid slots
                                    }
                                }}
                                style={{
                                    padding: 10,
                                    ...
                                    backgroundColor: isSelected ? "#0066b2" : "white",
                                    opacity: isTimeSlotPast ? 0.5 : 1, // Reduce opacity for past slots
                                }}
                                disabled={isTimeSlotPast} // Disable past slots
                            >
                                <Text
                                    style={{
                                        color: isSelected ? "white" : isTimeSlotPast ? "#a0a0a0" : "black", // Grey for past, white for selected, black for available
                                        fontSize: 14,
                                        fontWeight: "500",
                                        textAlign: 'center',
                                    }}
        ...
        return null; // Return null if no selected date
    };
    const renderTimeOptions = () => {
        return PickUpTimeOptions.map((option, index) => {
            ...
            // Return the time slot as a button
            return (
                <TouchableOpacity
                    key={index} // Correct syntax for the key
                    onPress={() => {
                        setSelectedDeliveryTime(option); // Set the selected time
                    }}
                    style={{
                        margin: 10,
                        padding: 10,
                        ...
                        backgroundColor:
                            selectedDeliveryTime &&
                                ...
                                ? "#0066b2" // Selected slot color
                                : "white", // Default slot color
                    }}
                >
                    <Text
                        style={{
                            ...
                            color:
                                selectedDeliveryTime &&
                                    selectedDeliveryTime.startTime === option.startTime &&
                                    selectedDeliveryTime.endTime === option.endTime
                                    ? "white" // Selected slot text color
                                    : "black", // Default slot text color
                        }}
                    >
                        {`${option.startTime} - ${option.endTime}`}
                    </Text>
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            >
                ...
                <Text style={{ flex: 1, fontSize: 16, fontWeight: "500" }}>Choose your address</Text>
                <View
                    style={{
                        width: 36,
                        height: 36,
